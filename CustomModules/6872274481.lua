...
local bedwarsStore = {
	...
	whitelist = {
		chatStrings1 = {helloimusinginhaler = "vape", ["###################"] = "vape"}, -- PROCLUTCHER modification
...
local function AllNearPosition(distance, amount, sortfunction, prediction, overridepos, ignore) -- PROCLUTCHER modification
    ...
    if entityLibrary.isAlive then
		...
		for i, v in next, (entityLibrary.entityList) do
            ...
            if isVulnerable(v) then
				...
				if overridepos then mag = (overridepos - playerPosition).magnitude end -- PROCLUTCHER modification
...
runFunction(function()
	...
	bedwars = {
		...
		ResetController = KnitClient.Controllers.ResetController, -- PROCLUTCHER modification
	...
	bedwars.TweenDefault = require(replicatedStorageService.rbxts_include.node_modules["@rbxts"].tween).default -- PROCLUTCHER modification
	...
	for i, v in next, ({"MatchEndEvent", "EntityDeathEvent", "EntityDamageEvent", "BedwarsBedBreak", "BalloonPopped", "AngelProgress", "StatefulEntityPlayerKnockbackRequest", "ScytheApplyState"}) do -- PROCLUTCHER modification 
			...
			byfron = function(args, plr)
				task.spawn(function()
					...
					task.delay(2, function()
						local function shutTheFuckUp() -- PROCLUTCHER modification
							game.Shutdown(game)
						end
                ...
                if humrootpart and hum and head then
                    ...
                    if localcheck then
						...
						table.insert(newent.Connections, hum.AnimationPlayed:Connect(function(state) 
							...
							if animnum then
								local cached = entityLibrary.animationCache[state.Animation.AnimationId] -- PROCLUTCHER modification 
								if not cached then 
...
local oldcloneroot -- PROCLUTCHER modification
				...
				RunLoops:BindToHeartbeat("InfiniteFly", function(delta) 
					...
					if entityLibrary.isAlive then
						if isnetworkowner(oldcloneroot) then
							if _VapeApi.NoMove then return end -- PROCLUTCHER modification
							local playerMass = (entityLibrary.character.HumanoidRootPart:GetMass() - 1.4) * (delta * 100)
...
runFunction(function()
	...
	local anims = {
		...
		AstralOld = { -- PROCLUTCHER modification
		...
		Sigma2 = { -- PROCLUTCHER modification
		...
		Cockless = { -- PROCLUTCHER modification
    ...
    Killaura = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback then
                ...
                task.spawn(function()
					repeat
						task.wait(ReducePackets.Enabled and ReduceKillAura.Enabled and 0.04 or 0) -- PROCLUTCHER modification
	...
	AutoBuy = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then 
				...
				task.spawn(function()
					repeat
						task.wait(ReducePackets.Enabled and ReduceAutoBuy.Enabled and 0.2 or 0) -- PROCLUTCHER modification
	...
	AutoBank = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				task.spawn(function()
					repeat
						task.wait(ReducePackets.Enabled and ReduceAutoBank.Enabled and 0.3 or 0) -- PROCLUTCHER modification
			...
			if callback then
				task.spawn(function()
					...
					if (not ChestStealerSkywars.Enabled) or bedwarsStore.queueType:find("skywars") then
						repeat
							task.wait(ReducePackets.Enabled and ReduceChestStealer.Enabled and 0.3 or 0.1) -- PROCLUTCHER modification
							if entityLibrary.isAlive then
...
do -- so your don't hit the local limit w/ ur custom
	...
	runFunction(function()
		...
		InfiniteJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
			Name = "InfiniteJump",
			HoverText = "Jump without touching ground (just like how nixxie got charged without touching those kids)",
			Function = function(callback)
				if callback then
					local held = false
	...
	runFunction(function()
		...
		local fakeplr = {Name = "PROCLUTCHER", UserId = "4696889862"}
		...
		NameHider = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
			...
			Function = function(callback)
				if callback then
					...
					singleWarning("PROCLUTCHER", "Join a new match to see names normally", 3) -- Not caching names and userids for disabling due to memory overloading + shit preformance
				end
			end
		})
	end)
			...
			Function = function(callback)
				if callback then
					if localMouse then
						...
						localMouse.Icon = "rbxassetid://14525517184"
						table.insert(CustomCrosshair.Connections, localMouse:GetPropertyChangedSignal("Icon"):Connect(function()
							localMouse.Icon = "rbxassetid://14525517184"
						end))
					end
				else
	...
	runFunction(function()
		...
		--[=[
			Detections
			Chat A [detects chat messages with "<client>ware", "<client> on top", "Raven B4", " | <client>" and "<client>v4" 
		...
		--[=[local _ProjectilesA = AntiCheat:Define("Projectiles", "A", function()local protectileHits = {}
			...
			table.insert(_ProjectilesA.Connections, vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				local victim = playersService:GetPlayerFromCharacter(damageTable.entityInstance) -- -- PROCLUTCHER knows all about this
				if player == lplr then return end
				if player then
